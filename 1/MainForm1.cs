...
namespace _1
{
    public partial class MainForm1 : Form
    {
        ...
        {
            string query = "select users.login as login, status.name as Name, jobs.name, ping.name from users join status on users.id_status = status.Id_status join jobs on users.id_jobs = jobs.Id_jobs join ping on users.id_ping = ping.Id_ping;";           
